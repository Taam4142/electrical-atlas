...
- Interactive visuals for the first electricity lesson, Electric Charge, Voltage, Electric Current, Resistance and Conductance, Ohm's Law, Series and Parallel Circuits, Switches and Contacts, Electrical Power and Energy, Battery, MOSFET switching, and Capacitance and Capacitors lessons.
...
Phase A1 corrected public status truth, invalid preview IDs, stale guidance, the root gateway, and unfinished-topic indexing behavior. Phase A2 established one reproducible root install path, strict inventory and relationship validation, generated-data drift checks, and a test-gated Vercel build. Phase B established the review-preparation and revision-binding workflow with Voltage and What Is Electricity, then applied it to Electric Charge and Capacitance and Capacitors. Exact commit `182a8b0` passed the rebound public-deployment check for all four review records. The full publication lifecycle is not yet proved because no lesson is governance-published: the capacitor candidate additionally retains qualified-human review, and every lesson still keeps project-owner Thai-language and final-publication gates separate. Diode is the next content milestone.
